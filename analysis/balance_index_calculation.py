...
# Category cuts on BI (0–1 scale)
CUTS = (0.05, 0.15, 0.30)
CAT_LABELS = ["Optimal", "Moderate", "High", "Severe"]


def _to_01(s: pd.Series) -> pd.Series:
    """Ensure a series is on 0–1 scale. If max>1 (e.g., 0–100), divide by 100."""
...
def compute_balance_index(df: pd.DataFrame, scale_percent: bool = True) -> pd.DataFrame:
    """
    Compute BI on 0–1 scale as |p_STEM - p_HSS|.
    If scale_percent=True, automatically converts 0–100 inputs to 0–1 by dividing 100.
    """
    ...
    # BI = |p_STEM - p_HSS| (0–1 scale; if input is 0–100, it is normalized to 0–1 first)
    out["BI"] = (stem - hss).abs()
    return out


def add_categories(df: pd.DataFrame, cuts=CUTS) -> pd.DataFrame:
    """Add categorical label for BI using fixed cuts on 0–1 scale."""
...
def _latest(df: pd.DataFrame, year_col: str = "year") -> pd.DataFrame:
    """
    Return the latest-year record per country.
    """
...
def summarize_latest(df_latest: pd.DataFrame) -> dict:
    """
    Summarize BI stats on latest-by-country subset + employment correlation (if available).
    """
    bi = pd.to_numeric(df_latest["BI"], errors="coerce").dropna()
    summary = {
        "bi_mean": float(bi.mean()) if len(bi) else None,
    ...
    # Employment correlation (if emp_25_34 exists)
    if "emp_25_34" in df_latest.columns:
        ...
        if mask.sum() >= 3:
            ...
            summary.update({
                "employment_r": r,
                "employment_r2": r ** 2,
                "employment_n": int(mask.sum()),
            })
    return summary


def run(in_panel: str = "outputs/panel_final.csv", outdir: str = "outputs/derived") -> dict:
    """
    Main pipeline:
      1) read panel csv
      2) compute BI (0–1)
      3) add categories
      4) write full panel (panel_with_bi.csv)
      5) keep latest per country -> balance_index_complete.csv
      6) write bi_summary.json
    ...
    # 1) full panel
    panel_path = outdir_path / "panel_with_bi.csv"
    df_bi.to_csv(panel_path, index=False)

    # 2) latest by country (legacy-like name)
    latest = _latest(df_bi).sort_values("BI").reset_index(drop=True)
    complete_path = outdir_path / "balance_index_complete.csv"
    latest.to_csv(complete_path, index=False)

    # 3) summary
    summary = summarize_latest(latest)
    summary_path = outdir_path / "bi_summary.json"
